...
1. [linear-data-structures.md](DSA/linear-data-structures.md) — **Linear Data Structures**: *Arrays & Strings* (prefix sums, two pointers, sliding window, Kadane's, 2D matrix, sorting essentials, hashing as a tool) · *Stacks & Queues* (brackets, monotonic stack/deque, expression eval, MinStack, BFS, circular queue)
...
3. [algorithms.md](DSA/algorithms.md) — **Algorithms**: *Binary Search* (bounds, search-on-answer, rotated array) · *Dynamic Programming* (linear, LIS, knapsack, string, state-machine, interval, game theory, grid, tree DP) · *Greedy* (intervals, sweep line, jump game, greedy + heap) · *Recursion / Backtracking / D&C* (backtracking templates, merge/quick sort, quickselect)
